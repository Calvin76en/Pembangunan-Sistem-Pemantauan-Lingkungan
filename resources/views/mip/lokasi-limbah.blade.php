...
<main>
  <div class="mx-auto max-w-7xl p-6">
    ...
    <!-- Header Card -->
    <!-- <div class="rounded-2xl p-8 mb-8 shadow-xl" style="background: rgba(186, 221, 186, 1);">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
          <h1 class="text-3xl font-bold text-white mb-2">Monitoring Air Limbah Tambang</h1>
          <p class="text-white/80">Kelola data pemantauan air limbah untuk setiap lokasi monitoring</p>
        </div>
        <div class="flex items-center bg-white/10 backdrop-blur-sm rounded-xl p-4">
          ...
          <div>
            <p class="text-white/80 text-sm">Tanggal Monitoring</p>
            <p class="text-white font-semibold">{{ \Carbon\Carbon::parse($selectedDate)->format('d M Y') }}</p>
          </div>
        </div>
      </div>
    </div> -->
      ...
      <div class="px-8 py-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p class="text-gray-600 dark:text-gray-400">Total {{ count($lokasi) }} lokasi </p>
          </div>

          <!-- Date Picker -->
          <form method="GET" action="{{ route('mip-lokasi-limbah') }}" class="flex items-center">
            <div class="relative">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
            @foreach($lokasi as $index => $location)
            @php
            // Debug: Ambil status dari array yang dikirim controller
            $status = $statusPerLokasi[$location->location_id] ?? 'empty';

            // Debug: Cek apakah ada data untuk lokasi ini
            $records = $dataByDate->get($location->location_id);
            $firstRecord = null;

            if ($records && $records->isNotEmpty()) {
            $firstRecord = $records->first(); // Ambil record pertama dari collection
            }

            // Debug: Pastikan location_id sesuai dengan yang ada di database
            // Dari database: 33000001, 33000002, 33000003, 33000004, 33000005
            @endphp
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200">
              ...
              <!-- Actions -->
              <td class="px-8 py-6 text-center">
                {{-- Debug Info - hapus setelah selesai troubleshoot --}}
                {{--
                <small class="text-xs text-gray-500 block mb-2">
                  Records: {{ $records ? $records->count() : '0' }} |
                FirstRecord: {{ $firstRecord ? 'Ada' : 'Kosong' }} |
                Status: {{ $status }}
                </small>
                --}}

                @if ($firstRecord || $status === 'completed' || $status === 'draft')
      ...
      <div class="px-8 py-6 bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div class="flex items-center text-sm text-gray-600 dark:text-gray-400">
            ...
            Total {{ count($lokasi) }} lokasi monitoring
          </div>

          <div class="flex items-center gap-6 text-sm">
            @php
            // Hitung status dari array yang sudah dikirim controller
